...
void State::update(long part, double u) {
	...
	long pos_next;
	if (part == 0) { // TP (backbone only)
		pos_next = pos + 2 * (u < probTP) - 1;
		if (pos_next == nbSitesX) {
			pos_next = 0;
			if (!occupations[pos_next]) // One turn was performed
				winding++;
		} else if (pos_next == -1) {
			pos_next = nbSitesX - 1;
			if (!occupations[pos_next]) // One turn was performed
	...
	} else if (pos >= nbSitesX) { // Particle on a tooth
		pos_next = pos + nbSitesX * (2 * (u < 0.5) - 1);
		if (pos_next >= nbSitesTot) 
			pos_next -= nbSitesTot;
		if (pos_next < 0) 
			pos_next += nbSitesTot;
	} else if (u < 0.5) { // Particle on the backbone moving vertically
		pos_next = pos + nbSitesX * (2 * (u < 0.25) - 1);
		// Only one boundary condition
		if (pos_next < 0) 
			pos_next += nbSitesTot;
	} else { // Particle on the backbone moving horizontally
		pos_next = pos + 2 * (u < 0.75) - 1;
		if (pos_next == nbSitesX)
			pos_next = 0;
		else if (pos_next == -1)
			pos_next = nbSitesX - 1;
